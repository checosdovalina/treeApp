...
export default function PromotionBanner({ 
  ...
  return (
    <div className="relative w-full overflow-hidden">
      ...
      {promotions.length > 1 && (
        <div className="absolute bottom-4 sm:bottom-6 md:bottom-8 lg:bottom-10 left-1/2 transform -translate-x-1/2 flex space-x-2 sm:space-x-3 bg-black/50 backdrop-blur-xl px-3 py-2 sm:px-4 sm:py-2 md:px-5 md:py-3 rounded-full border border-white/40 shadow-2xl">
          {promotions.map((_, index: number) => (
            <button
              key={index}
              className={`w-2 h-2 sm:w-2.5 sm:h-2.5 md:w-3 md:h-3 lg:w-3.5 lg:h-3.5 rounded-full transition-all duration-500 border shadow-lg ${
                index === currentIndex 
                  ? 'bg-gradient-to-r from-yellow-300 via-yellow-400 to-orange-400 border-yellow-200 scale-125 shadow-yellow-400/70' 
                  : 'bg-white/60 border-white/80 hover:bg-white/90 hover:scale-110 hover:border-white hover:shadow-white/40'
              }`}
              onClick={(e) => {
                e.stopPropagation();
